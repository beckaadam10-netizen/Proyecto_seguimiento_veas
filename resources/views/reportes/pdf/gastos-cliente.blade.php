...
<head>
    <meta charset="utf-8">
    <title>Detalle Gastos Judiciales</title>
    <style>
    ...
    @else
        ...
        <div class="bloque-caso">
            <table class="datos-doc">
                <tr><td><strong>Código:</strong> {{ $grupo->codigo_display }}</td></tr>
                <tr><td><strong>Cliente:</strong> {{ $grupo->cliente?->nombre_completo ?? '—' }}</td></tr>
                <tr><td><strong>Número de proceso:</strong> {{ $grupo->titulo_display }}</td></tr>
                @if($grupo->instancia)
                <tr><td><strong>{{ $grupo->tipo === 'expediente' ? 'Juzgado' : 'Institución' }}:</strong> {{ $grupo->instancia }}</td></tr>
                @endif
                @if($grupo->abogado)
                <tr><td><strong>Abogado encargado:</strong> {{ $grupo->abogado }}</td></tr>
                @endif
