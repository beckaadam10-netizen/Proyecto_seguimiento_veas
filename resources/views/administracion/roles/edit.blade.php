...
@section('header', 'Administración / Editar Rol')

@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('administracion.roles.update', $rol) }}" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="descripcion" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-400">{{ old('descripcion', $rol->descripcion) }}</textarea>
            </div>
            <div>
                ...
                @include('administracion.roles._matriz-permisos', [
                    ...
                    'seleccionados'   => old('permisos', $rol->permisos->pluck('id')->all()),
                    'idPrefix'        => 'editar-' . $rol->id,
                ])
            </div>
            <div class="flex items-center gap-2">
                <input type="hidden" name="activo" value="0">
                <input type="checkbox" name="activo" id="activo" value="1" {{ old('activo', $rol->activo) ? 'checked' : '' }}
